...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/adpf/tracking/click/getList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#ickAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#ickAdd").data('action-url');
		top.app.layer.editLayer('新增点击日志', ['710px', '580px'], '/tracking/click/t-click-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
	$("#ickEdit").click(function () {
		...
		params.type = 'edit';
		params.rows = rows[0];
		params.operUrl = top.app.conf.url.apigateway + $("#ickEdit").data('action-url');
		top.app.layer.editLayer('编辑点击日志', ['710px', '580px'], '/tracking/click/t-click-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
	$("#ickDel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.delData($table, $("#ickDel").data('action-url'), idsList);
    });
}
