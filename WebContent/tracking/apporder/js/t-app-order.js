...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/adpf/tracking/apporder/getList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#tAppOrderAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#tAppOrderAdd").data('action-url');
		top.app.layer.editLayer('新增订单日志', ['710px', '520px'], '/tracking/apporder/t-app-order-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
	$("#tAppOrderEdit").click(function () {
		...
		params.type = 'edit';
		params.rows = rows[0];
		params.operUrl = top.app.conf.url.apigateway + $("#tAppOrderEdit").data('action-url');
		top.app.layer.editLayer('编辑订单日志', ['710px', '520px'], '/tracking/apporder/t-app-order-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
	$("#tAppOrderDel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.delData($table, $("#tAppOrderDel").data('action-url'), idsList);
    });
}
